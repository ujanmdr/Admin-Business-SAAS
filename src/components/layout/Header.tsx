...
import { Bell, ChevronDown, Menu, Plus, Search, MapPin, Building2, LayoutDashboard, ShieldAlert } from "lucide-react";
...
export function Header() {
  ...
  return (
    <>
      ...
      <header className={`sticky z-30 bg-background/85 backdrop-blur border-b border-border ${isSuspended ? 'top-9' : 'top-0'}`}>
        <div className="flex items-center gap-3 px-4 lg:px-8 h-16">
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            <SheetTrigger asChild>
              <button className="lg:hidden h-9 w-9 grid place-items-center rounded-lg hover:bg-muted">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button 
              ...
              className={`hidden sm:inline-flex items-center gap-2 rounded-xl text-primary-foreground px-3.5 py-2 text-sm font-medium transition ${
                isSuspended ? 'bg-muted text-muted-foreground cursor-not-allowed opacity-50' : 'bg-primary shadow-luxe hover:opacity-95'
              }`}
            >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-2 rounded-xl border border-border bg-card pl-1 pr-3 py-1 hover:bg-muted">
                ...
                <div className="hidden md:block text-left leading-tight">
                  <div className="text-sm font-medium">Pratiksha S.</div>
                  <div className="text-[10px] text-muted-foreground">Owner</div>
                </div>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuLabel>Pratiksha Shrestha</DropdownMenuLabel>
                <DropdownMenuSeparator />
